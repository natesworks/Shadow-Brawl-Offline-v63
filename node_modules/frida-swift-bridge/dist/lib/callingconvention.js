...
import { makeBufferFromValue, makeValueFromBuffer, moveValueToBuffer, } from "./buffer.js";
...
class TrampolinePool {
    ...
    static allocateTrampoline(size) {
 ...
 */
export function makeSwiftNativeFunction(address, retType, argTypes, context, throws) {
    const loweredArgType = argTypes.map((ty) => lowerSemantically(ty));
    const loweredRetType = lowerSemantically(retType);
    const swiftcallWrapper = new SwiftcallNativeFunction(address, loweredRetType, loweredArgType, context).wrapper;
    const wrapper = function (...args) {
        ...
        for (const [i, arg] of args.entries()) {
            ...
            if (!composition.isClassOnly) {
                container = TargetOpaqueExistentialContainer.alloc(composition.numProtocols);
                container.type = typeMetadata;
                if (typeMetadata.isClassObject()) {
                    container.buffer.privateData.writePointer(arg.handle);
                }
                else {
                    const box = typeMetadata.allocateBoxForExistentialIn(container.buffer);
                    typeMetadata.vw_initializeWithCopy(box, arg.handle);
                }
            }
            else {
                container = ClassExistentialContainer.alloc(composition.numProtocols);
        ...
        if (retType instanceof TargetMetadata) {
            switch (retType.getKind()) {
                case MetadataKind.Struct:
                    return new StructValue(retType, {
                        raw: retval,
                    });
                case MetadataKind.Enum:
                    return new EnumValue(retType, {
                        raw: retval,
                    });
                case MetadataKind.Class:
                    return new ObjectInstance(retval);
                default:
...
function lowerSemantically(type) {
    ...
    const layout = type.getTypeLayout();
    /**TODO:
...
function lowerPhysically(value) {
    ...
    return makeValueFromBuffer(value.handle, value.$metadata.getTypeLayout().stride);
}
export function shouldPassIndirectly(typeMetadata) {
...
export class SwiftcallNativeFunction {
    ...
    #extraBuffer;
    #nativeFunction;
    constructor(target, resultType, argTypes, context, errorResult) {
        this.#argumentBuffers = new StrongQueue();
        argTypes = argTypes
            .map((argType) => {
